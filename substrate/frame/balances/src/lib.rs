...
pub mod pallet {
	...
	use crate::types::{validate_transaction, LocalAuthority, PostContractInterface};
	use frame_support::{
		pallet_prelude::*,
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		type PostContractInterface: PostContractInterface<Self::AccountId>;
	...
	/// Address of the contract. Given the same salt and account to instantiate the contract,
	/// the address of the contract remains the same.
	#[pallet::storage]
	#[pallet::getter(fn contract_address)]
	pub type PostsContractAddress<T: Config<I>, I: 'static = ()> =
	StorageValue<_, Option<T::AccountId>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn posts_count)]
	pub type PostsContractPostsCount<T: Config<I>, I: 'static = ()> = StorageValue<
		_,
		<T::PostContractInterface as PostContractInterface<T::AccountId>>::PostCount,
		ValueQuery,
	>;
	...
	pub struct GenesisConfig<T: Config<I>, I: 'static = ()> {
		...
		pub post_contract_address: Option<T::AccountId>,
	}

	impl<T: Config<I>, I: 'static> Default for GenesisConfig<T, I> {
		fn default() -> Self {
			Self { balances: Default::default(), dev_accounts: None, post_contract_address: Default::default() }
		}
	}
	...
	impl<T: Config<I>, I: 'static> BuildGenesisConfig for GenesisConfig<T, I> {
		fn build(&self) {
			...
			<PostsContractAddress<T, I>>::put(self.post_contract_address.clone());
			...
			assert_eq!(endowed_accounts.len(), self.balances.len(), "duplicate balances in genesis.");
		...
		T: CreateTransactionBase<Call<T, I>> + CreateBare<Call<T, I>>,
	{
		...
		fn offchain_worker(block_number: BlockNumberFor<T>) {
			if sp_io::offchain::is_validator() && Self::should_fund() {
				T::PostContractInterface::local_authority_key()
					.and_then(|(index, key)| {
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			match call {
				Call::assess_funding { authority_index, signature } if Self::should_fund() =>
					validate_transaction::<T, T::PostContractInterface>(
						authority_index,
						signature,
						"PostsContractValidation",
					),
				...
				Call::set_contract_address { .. } => InvalidTransaction::Call.into(),
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn assess_funding(
			...
			_signature: <<T::PostContractInterface as LocalAuthority>::AuthorityId as RuntimeAppPublic>::Signature,
		) -> DispatchResult {
			// unsigned
			ensure_none(origin)?;
			if Self::should_fund() {
				let contract_address =
					Self::contract_address().ok_or(Error::<T, I>::ContractAddressNotSet)?;
				let current_block_number = <frame_system::Pallet<T>>::block_number();

				// shouldn't happen since the offchain worker was called in the previous block
				ensure!(
                    current_block_number > authority_index.block_number,
                    Error::<T, I>::InvalidBlockNumber
                );
				let current_post_count = T::PostContractInterface::get_post_count(contract_address);
				let previous_count = Self::posts_count();
				ensure!(current_post_count >= previous_count, Error::<T, I>::InvalidPostCount);
				let limit = T::PostContractInterface::FUNDING_THRESHOLD;
				if current_post_count.saturating_sub(previous_count) >= limit {
					<FreeFunding<T, I>>::put(false);
					log::info!("ceased");
					Self::deposit_event(Event::FundingCeased);
				} else {
					<PostsContractPostsCount<T, I>>::put(current_post_count)
				};
				Ok(())
			} else {
		...
		/// Method used by the **TESTS** to set the contract address
		#[pallet::call_index(12)]
		#[pallet::weight(Weight::from_all(100_000_000_000))]
		pub fn set_contract_address(
			origin: OriginFor<T>,
			_test_contract_address: T::AccountId,
		) -> DispatchResult {
			ensure_signed(origin)?;
			#[cfg(feature = "contract-test")]
			{
				<PostsContractAddress<T, I>>::put(Some(_test_contract_address));
				log::info!("Contract address set");
			}
			#[cfg(not(feature = "contract-test"))]
			{
				log::info!("Calling this method only decreases your funds");
			}
			Ok(())
		}
	}
...
impl<T: Config<I>, I: 'static> OnNewAccount<T::AccountId>  for Pallet<T, I> {
